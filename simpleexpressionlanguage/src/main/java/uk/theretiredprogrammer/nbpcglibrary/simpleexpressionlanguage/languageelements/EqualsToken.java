...
/**
 *  The Equals Operator (==). Compares two strings for equality.
 * 
...
public class EqualsToken extends OperatorToken {
    ...
    public ParseTree getParseTree(ParseTree lower, ParseTree upper) {
        return new EqualsOperation(lower, upper);
    }

    private class EqualsOperation extends ParseTreeBooleanResult {

        private final ParseTree left;
        private final ParseTree right;

        public EqualsOperation(ParseTree left, ParseTree right) {
            this.left = left;
            this.right = right;
        }
