 ...
 */
public class FolderBackingObject implements FieldBackingObject<String> {
     ...
     */
    public FolderBackingObject(String label, String initialvalue) {
        backingString = initialvalue;
        this.label = label;
        rules.addRule(new FolderExistsRule());
    }
     ...
     * @param label the fields label - for reporting
     */
    public FolderBackingObject(String label) {
        this(label, "");
    }
    ...
    private class FolderExistsRule extends Rule {
        ...
        public FolderExistsRule() {
            super(label + " - file does not exist or is a file");
        }

        @Override
        public boolean ruleCheck() {
            File folder = new File(backingString);
