 ...
 */
public abstract class GetAllServlet<T extends BasicEntity> extends CommonPostServlet {
     ...
     */
    public GetAllServlet(String tablename) {
    ...
    protected void processCommand(JsonObject parameters, JsonGenerator replyGenerator) {
        try {
            ...
            replyGenerator.write("success", true).write("count", entities.size());
            replyGenerator.writeStartArray("entities");
            entities.stream().forEach((entity) -> {
                entity.writeAllFields(replyGenerator, null);
            });
