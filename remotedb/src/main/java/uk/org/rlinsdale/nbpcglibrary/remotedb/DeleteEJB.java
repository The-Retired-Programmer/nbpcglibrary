...
/**
 * Create a EJB to handle delete requests.
 *
 * @author Richard Linsdale (richard.linsdale at blueyonder.co.uk)
 * @param <T> the remote entity type
 * @param <K> the primary Key class
 */
public abstract class DeleteEJB<T extends BasicEntity, K> implements CommonCommandEJB<T> {
    ...
    public void processCommand(Class<T> clazz, JsonObject parameters, JsonGenerator replyGenerator) {
        ...
        if (pkey == null) {
            ...
            try {
                T entity = getEntityManager().find(clazz, pkey);
                if (entity != null) {
                    System.out.println("entity found");
                    this.getEntityManager().remove(entity);
                    System.out.println("entity removed");
                    replyGenerator.write("success", true);
                    entity.writePKwithkey(replyGenerator);
